...
## ⏸ 当前进度断点（2026-09-26 挂起，明日续接）

**已完成**：
- ✅ 微博账号「旭里有时掺点虚」（UID **5212556848**）登录开放平台
- ✅ 开发者实名认证（扫脸通过）
- ✅ 7 天免费体验包已领取（2026-09-26 起，**读 5 次/写 3 次**——
  share.json 发布消耗"写"配额，验证测试 1 条+真实发布最多 2 条）
- ✅ 开发者基本信息已提交（元禾生物科技/山东泰安/
  邮箱 710933721@qq.com）
- ⬜ **邮箱验证未生效**——验证邮件发送次数今日达上限挂起

**明日续接步骤**（说「继续微博 token 任务」即可）：
1. 重新触发邮箱验证邮件 → **在保有开放平台登录态的同一浏览器内**
   新开标签页登录 QQ 邮箱点验证链接（手机/他浏览器点不生效，
   今日实证）
2. 验证生效 → 创建网页应用（agent 代填表单：应用名
   「竹香酒官方推广」/地址 zxjiu.com/回调
   https://zxjiu.com/weibo-callback，用户确认提交）
3. 取 App Key / App Secret
4. 构造授权 URL（client_id=KEY&redirect_uri 编码）→ 用户
   浏览器授权 → 取回调 code（10 分钟有效）
5. 服务器 curl `oauth2/access_token` 换 token
6. `bash /root/deploy_weibo_token.sh '<token>'`（生产已部署：
   验证 token→写 .env→rebuild→断言 effectiveMode=real）

**时效提醒**：体验包 7 天窗口自 09-26 起算（明日剩 6 天）；
浏览器代理断点 ID `10060c99-ed5d-47f2-a6c4-31ea3d0a24dd`
（明日可 resume 或新启）。
