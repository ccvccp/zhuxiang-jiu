...
from routes import (
    ...
    register_zy_routes,
    # 智法·AI智能法务大模型(24号合规模块升级更名·产销法一体化)
    register_zf_routes,
...
# 智法·AI智能法务大模型(24号合规模块升级更名·产销法一体化)
register_zf_routes(app)
# 智运·AI智能物流大模型(物流接口管理模块升级更名·智能调度中枢)
register_zw_routes(app)
register_zt_routes(app)
# 智单·AI智能订单大模型(订单管理模块升级)
...
async def _on_startup():
    ...
    start_zt_map_guard()
    # 智能产品管理大模型: 护栏指标自动巡检
    # (PDM_GUARD_AUTO=off 可关闭, 默认 on——护栏是保护机制)
    from services.pdm_scheduler import start_guard_loop as start_pdm_guard
    start_pdm_guard()
    # 条款协议大模型: 护栏指标自动巡检
    # (AGREEMENT_GUARD_AUTO=off 可关闭, 默认 on——护栏是保护机制)
    from services.agreement_scheduler import start_guard_loop as start_agreement_guard
    start_agreement_guard()
    # 图标主题大模型: 护栏指标自动巡检
