...
class TestSync:
    async def run(self):
        ...
        record("29档案全量入册",
               ...
               and len(SCORER_REGISTRY) == 38,
               f"added={r.get('added')} "
               f"total={len(SCORER_REGISTRY)}")
        record("批次覆盖1-22", r["discovered"] == 38,
        ...
        record("台账统计", reg["total"] == 38
               and reg["byStatus"].get("active") == 38,
               str(reg.get("byStatus")))
        record("批次分布", sum(
            (reg.get("byBatch") or {}).values()) == 38,
               str(reg.get("byBatch")))
