"""55号·二维码AI智能管理 自主学习进化
(qr55_learn_service, P3)

计划(docs/55号_二维码AI智能管理模块实施计划.md §六 P3):
    - Hedge 学习轮次(44号 run_learning_cycle 复用——
      不重造轮子)+护栏验证([0.5,2.0] 倍+归一化)
    - 自主升级: 晋升+版本快照(qr55_model_events
      ——审计+回滚依据)
    - 自主回滚: 滑动窗口指标回退检测 → 自动回退
      上一版本+冻结学习+告警
    - 手动通道: promote/rollback 端点(人工兜底)

设计(44号薄包装层, 54号 Login54LearnService P3
范式全继承):
    - run_learning: 包装 run_learning_cycle
      (qr_orchestration) → qr55_model_events
      留痕(learning/promoted 两类事件)——版本溯源
    - shadow_compare: challenger vs champion 对
      同一上下文的双轨试算对比(生码策略档位差异
      ——shadow 语义, 不落库不生效)
...
QC(计划 §六 P3):
    - 权重演进单调收敛(护栏 [0.5,2.0] 倍+和=1.0)
    - 晋升-回滚闭环: 手动晋升→回滚→版本历史可溯

零侵入红线: 44号学习引擎零改动(纯调用式包装
...
logger = logging.getLogger("qr55_learn_service")

MODEL_VERSION = "v1-qr55-learn"

SCORER_ID = "qr_orchestration"

# 影子对比差异语义(生码策略档位序:
# clarify 保守 → confirm 中间 → direct 信任)
STRATEGY_ORDER = {"clarify": 0, "confirm": 1,
                  "direct": 2}
...
def _regression_threshold() -> float:
    """回归阈值(环境变量可覆盖——QR55_REGRESSION_THRESHOLD)"""
    import os
    try:
        return abs(float(os.environ.get(
            "QR55_REGRESSION_THRESHOLD",
            str(REGRESSION_THRESHOLD))))
    except ValueError:
        return REGRESSION_THRESHOLD


class Qr55LearnService:
    """55号自主学习进化(44号 Hedge 引擎包装+
    模型事件留痕+影子对比)"""
        ...
        try:
            ...
            await Qr55Service().record_model_event(
                "promoted" if promoted else "learning", {
                    ...
                    "championMetrics":
                        result.get("championMetrics"),
                    "challengerMetrics":
                        result.get("challengerMetrics"),
                })
        except Exception as exc:  # noqa: BLE001
            # 留痕 fail-soft——学习本体不受影响
            logger.warning(
                "qr55_learn_event_failed: %s", exc)
    ...
    async def shadow_compare(self, ctx: dict) -> dict:
        ...
        # champion 轨(当前生效口径)
        champion_result = await Qr55Scorer().score(ctx)
        ...
        if challenger_result is not None:
            c_strat = champion_result.get("strategy")
            h_strat = challenger_result.get("strategy")
            diff = (STRATEGY_ORDER.get(h_strat, 0)
                    - STRATEGY_ORDER.get(c_strat, 0))
            if diff > 0:
                verdict = "challenger更宽(信任倾向)"
            elif diff < 0:
                verdict = "challenger更严(保守倾向)"
            else:
                verdict = "档位一致"
            comparison = {
                "championStrategy": c_strat,
                "challengerStrategy": h_strat,
                "strategyDiff": diff,
                "verdict": verdict,
                "championTrust":
    ...
    async def _score_with(weights: dict,
        ...
        try:
            return await Qr55Scorer().score(ctx)
        finally:
    ...
    async def promote(self) -> dict:
        ...
        try:
            ...
            logger.warning(
                "qr55_promote_event_failed: %s", exc)
        result.update({
            "module": "qr55",
            "note": "手动晋升通道(44号引擎复用)",
            "promotedAt": ts(),
    ...
    async def rollback(self, version_id: str = None,
        ...
        try:
            ...
            logger.warning(
                "qr55_rollback_event_failed: %s", exc)

        logger.info("qr55_rollback %s→%s(target=%s)",
    ...
    async def check_regression(self) -> dict:
        ...
        try:
            ...
            logger.warning(
                "qr55_regression_event_failed: %s", exc)
        result.update({
    ...
    async def _freeze_for_regression(
        ...
        try:
            ...
            review = await gov.review_change(
                change["changeId"], True, "qr55-auto")
            return {"frozen": True,
                    "changeId": change["changeId"],
                    "review": review.get("status")}
        except Exception as exc:  # noqa: BLE001
            logger.warning(
                "qr55_regression_freeze_failed: %s", exc)
            return {"frozen": False,
                    "error": str(exc)[:80]}
