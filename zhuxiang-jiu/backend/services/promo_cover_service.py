"""36号·RPA 图文封面生产(确定性品牌卡片·Pillow·多平台)

背景(2026-09-26 小红书立项): 图文笔记强制要求至少 1 张图
——RPA 发布依赖页面"文字配图"按钮曾需人工干预, 不可持续。
本服务在发布前确定性渲染竹绿主题品牌卡片(零外部 API/秒级/
风格固定可审), RPA 下载后自动上传, 全链无人干预。
2026-09-27 抖音图文接入: 1080×1440(3:4)对抖音图文竖版
同样适用, 按内容渲染与平台无关, 直接复用。

设计口径(与站点主题一致):
    - 尺寸 1080×1440(小红书/抖音图文 3:4 竖版)
    - 竹绿 #355c44 × #4a7c59 + 金 #c9a961 + 米白底 #f6f4ee
    - 标题与发布版一致(20 字限口径), 超长换行
    - 底部健康警示 + "详情见主页"(合规尾巴)
