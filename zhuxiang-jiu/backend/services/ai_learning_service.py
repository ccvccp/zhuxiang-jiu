...
SCORER_REGISTRY = {
    ...
    # ---- 第八批(41号AI智能代驾 P0/P1/P3: 司机资格审查+智能派单+评价审评) ----
    ...
    # ---- 第十五批(56号AI智能升级管理 P0: 升级编排评分) ----
    "upgrade_orchestration":        {"label": "智能升级编排评分", "module": "56升级管理", "batch": 15},
    # ---- 第十六批(57号AI智能知识库 P0: 知识编排评分) ----
...
DECISION_THRESHOLDS = {
    ...
    "qr_orchestration":      [(70.0, "direct"), (40.0, "confirm"),
                              (0.0, "clarify")],
    # 56号AI智能升级管理(三级决策——计划 §六:
    # 高分=高必要性+高信任 → propose 提案/escalate
    # 紧急提案+双人复核/defer 观察)
    "upgrade_orchestration": [(80.0, "escalate"), (50.0, "propose"),
                              (0.0, "defer")],
    # 57号AI智能知识库(三级决策——计划 §六:
    # 高分=高必要性+高信任 → collect 定向采集
    # /urgent 紧急采集+人工加急复审/defer 观察)
    "knowledge_orchestration": [(80.0, "urgent"), (50.0, "collect"),
                               (0.0, "defer")],
    # 58号AI智能优化意图识别(三级决策——计划 §五:
    # 高分=高语料质量+高置信度 → optimize 优化执行
    # /urgent 紧急优化+人工加急(Bad Case 复盘会)
    # /observe 观察)
    ...
    "service_orchestration": [(80.0, "urgent"), (50.0, "optimize"),
                              (0.0, "observe")],
    # 63号AI智能后台管理(三级决策——计划 §四:
    # 高分=高护航有效性+高审核一致性 → optimize 优化执行
    # /urgent 紧急优化+人工加急(后台复盘会)
    # /observe 观察)
    ...
    "payment_orchestration": [(80.0, "urgent"), (50.0, "optimize"),
                              (0.0, "observe")],
    # 61号AI智能系统升级决策(三级决策——计划 §四:
    # 高分=高决策准确率+高预警有效 → optimize 优化执行
    # (阈值与图谱回流) /urgent 紧急优化+决策复盘会
    # /observe 观察(建议域收窄))
...
def default_weights(scorer_id: str) -> dict:
    ...
    if scorer_id == "ride_dispatch":
        # 41号智能代驾: 派单适配五因子, 单一事实源在评分器类
        from services.ai_scoring_service import RideDispatchScorer
        return dict(RideDispatchScorer.WEIGHTS)
    if scorer_id == "ride_review":
    ...
    if scorer_id == "trust_value":
        ...
        return dict(TrustValueScorer.WEIGHTS)
