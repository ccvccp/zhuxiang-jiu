...
class TestScan:
    async def run(self):
        ...
        r = await svc.scan()
        record("巡检全档案(batch48=64)", r["success"] is True
               and r["scorerCount"] == 64,
        ...
        record("快照落库可读回",
               snap is not None
               and snap["scorerCount"] == 64
               and isinstance(snap.get("entries"), list)
               and len(snap["entries"]) == 64,
...
class TestAlerts:
    async def run(self):
        ...
        record("live排行结构",
               ...
               and live["scorerCount"] == 64
               and len(live["entries"]) == 64,
