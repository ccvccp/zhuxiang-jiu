...
class TestScan:
    async def run(self):
        ...
        record("快照落库可读回",
               snap is not None
               and snap["scorerCount"] == 38
               and isinstance(snap.get("entries"), list)
               and len(snap["entries"]) == 38,
...
class TestAlerts:
    async def run(self):
        ...
        record("live排行结构",
               ...
               and live["scorerCount"] == 38
               and len(live["entries"]) == 38,
