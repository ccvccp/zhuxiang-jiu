...
> 日期: 2026-09-16 | 版本: **终版二刷**(含验收后 H5 真实定位业务修正) | 验收范围: 大模型二代转段全链交付(三态灰度+护栏+控制面+自动巡检+62号同步桥接)
...
| fb60c21 | 上线验收报告(初版) | ✅ 已推送 |
| d91d424 | 位置地图管理模块上线验收报告(模块整体三层结构终态, 与本报告配套) | ✅ 已推送 |
| 550236c | 验收报告终版一刷(补全文档链) | ✅ 已推送 |
| 47eed7e | **验收后业务修正**: H5 接入真实定位(修复打开后无法定位当前位置, 详见 5.4) | ✅ 已推送 |

> 关联文档: 本报告聚焦智图大模型层(28 端点)验收; 模块整体
> (基础 13 端点+大模型 28 端点+一代 20号决策门)终态验收见
> 《位置地图管理模块_上线验收报告》(d91d424, 终版 82d1b63)。
...
### 5.4 验收后业务修正: H5 接入真实定位(47eed7e, 终版二刷纳入)

> 用户验收后实测发现——本模块上线后的首项前端消费面
> 缺陷修正实录。

- **现象**(用户实测): 智图 AI 地图打开后定位不了
  现在的真实位置
- **根因**(一代交付遗留): 前端页面全功能写死春熙路
  演示锚点(`CENTER` 常量), **从未实现定位调用**——
  地图中心/时空搜索/附近 POI/多仓匹配/沙盘推演均以
  固定坐标运行
- **修复**(双轨定位):
    - 优先 `BMap.Geolocation`(百度定位)——BD09 坐标系
      与地图/POI 完全一致无偏移, 浏览器定位不可用时
      自动 IP 定位
    - 未配 AK(列表降级模式)降级 `navigator.geolocation`
      (WGS84, 列表距离口径够用)
    - 定位成功: 动态中心 + 地图 panTo + 标记"我的位置";
      失败区分权限拒绝/超时/环境不支持提示
    - 全功能切换动态 center; 未定位时诚实展示
      "演示锚点(春熙路)"降级说明
- **部署**: H5 重建上传 `/var/www/zxjiu/dist`;
  生产实证智图 chunk 含 BMap.Geolocation+
  getCurrentPosition+定位按钮
- **沉淀**: 地图定位坐标系一致性是关键(BD09 地图配
  WGS84 原生定位会偏移数百米); 演示锚点交付后须
  及时接入真实定位
...
6. **验收后修正闭环**(终版二刷): 三处修正均为部署/实机/
   用户实测实证驱动——护栏口径(生产自锁→已处置分母) +
   月台视图(种子虚拟→fabric 合并视图) + **H5 真实定位**
   (演示锚点→BMap.Geolocation 双轨定位, 用户实测发现);
   前两处修正后专项断言扩充实证 165 断言复跑全绿,
   定位修正后生产 chunk 实证生效
