# 68号·信值·臻选购物平台——运行报告

> 范围: 五期交付(P0-P5)的运行时行为、实机验证证据与灰度运行口径
> 环境: Docker 容器(Redis 模式, LOCK_MODE=redis/STORE_MODE=redis)
> 报告时间: 2026-09-11(提交 016a2e5 实机验证轮)

---

## 一、运行环境与部署拓扑

```
宿主机(docker-compose.yml, 项目名 zhuxiang-jiu)
├── backend(zhuxiang-jiu-backend 镜像, 端口 8000)
│   ├── uvicorn main:app --workers 1
│   ├── XINZHI_MODE=${XINZHI_MODE:-off}   ← 68号灰度总开关(默认零影响)
│   └── 健康检查 /api/decision/health(10s 间隔)
└── redis(7-alpine, 端口 6379, appendonly 持久化)
    └── 7 张 xinzhi_* 表(Hash 结构, bool 显式还原)
```

**部署命令**(中文目录必须带 `-p`):
```powershell
docker compose -p zhuxiang-jiu -f docker-compose.yml up -d --build backend
```

**验收脚本**(宿主机执行, 直达容器网络):
```powershell
python backend/verify_xinzhi_p5_live.py   # 56 断言全链
```

---

## 二、灰度运行口径(P5)

### 2.1 三态与读取链

| 态 | 语义 | 生效范围 |
|----|------|---------|
| `off`(默认) | 决策面关闭(409) | 6 决策面拒绝; 观测面照常 |
| `shadow` | 观察学习期 | 决策放行, 响应携带 `xinzhiMode: shadow` 留痕 |
| `assist` | 辅助生产期 | 决策生效, 响应携带 `xinzhiMode: assist` |

**读取优先级**: `护栏暂停(guard_pause) > 运行时 override > 环境变量 XINZHI_MODE > 默认 off`

运行时切换(人工留痕, 不需重建容器):
```
POST /api/xinzhi/mode/override?mode=assist   # 切档
POST /api/xinzhi/mode/override?mode=         # 清除(回落 env)
GET  /api/xinzhi/mode                        # 总览(当前态+护栏+决策面清单)
```

### 2.2 决策面/观测面清单

| 类别 | 端点 | off 态行为 |
|------|------|-----------|
| **决策面(6)** | groupbuy 发布/响应/关闭、merchant apply/regrade、guide | 409(决策面关闭) |
| **观测面(永不关停)** | radar、radar/history、products/score、prime、price、feedback(POST/GET)、guide/personas、neighbor、groupbuy GET(大厅)、carbon、merchant simulate/level、mode、whitepaper | 200 正常 |

### 2.3 A/B 护栏(自动暂停机制)

- **监控三指标**: 退款率 / 客诉进线率 / 卸载率(基线 0.05/0.02/0.01)
- **触发条件**: 任一指标 `(当期-基线)/基线 > 3%` → **自动暂停**(等效 off)
- **性质**: 保护机制(功能开关非处罚)——暂停可自动, 恢复须人工:
  ```
  POST /api/xinzhi/mode/guard   # 检查(指标留痕滚动, 截断 50 条)
  POST /api/xinzhi/mode/resume # 人工恢复(决策留痕)
  ```
- **实机证据**: 退款率 0.05→0.10(+100%>3%)→ 自动暂停 → 求购 409 →
  人工 resume → 回 assist 档 → override 清除 → 回落 env=off

---

## 三、实机验证证据(56 断言×3 轮幂等)

### 3.1 关键链路实测值

| 链路 | 实测 | 断言 |
|------|------|------|
| 雷达即时计算 | 五维产出(诚信30/互助25/专业20/活跃15/成长10) | 5 |
| 播种等级控制 | S 级生效(radar GET 返回 grade=S) | 1 |
| 商品三维评分 | ZX42-2026L07 批次评分+可解释(fit/safety/conversion) | 5 |
| **价格构成(杀熟链)** | trusted+S 实付 ¥216.41(α=0.15 抵扣 ¥38.19); restricted+D 实付 ¥281.4(零抵扣); **价差 23%>20% → auditFlag 留痕** | 4 |
| 地板保护 | promo=0.5 时 floored=true, finalPrice=¥187.6(268×0.7) | 1 |
| 反馈闭环 | L2 信值产品组路由/L3 紧急 15 分钟/L1 自动回复/越权 404 | 6 |
| 灰度门槛 | 默认 off; 导购/求购/商家认证三决策面 409; 观测面 200 | 6 |
| assist 放行 | 导购 SOP 五步(steps=5+xinzhiMode=assist)/求购全链 | 12 |
| 46号总线 | registry/sync 入册 41+ 档案(xinzhi_merchant batch 41) | 1 |
| 商家体系 | 4+2 认证 A 档(certScore=54)/缺必查拒绝/沙盘 1000 单/评级 hold | 6 |
| 邻里频道 | 匿名门槛 K=5 公示/零个体数据(无 memberId/nickname/phone) | 2 |
| 护栏 | 正常不暂停/+100% 暂停/暂停后决策面关闭/guard_pause 总览 | 4 |
| 白皮书 | 四章节/PII 零命中/年度聚合含 67号联动 | 3 |
| 收官复位 | override 清除回落 env=off(零影响铁律闭环) | 1 |

### 3.2 幂等性(×3 轮)

- 同脚本连续 3 轮全绿(Redis 残留数据不破坏断言):
  唯一手机号(时间戳)注册/雷达快照累加/latest 取最新/灰度态
  复位键删除(`zhuxiang:xinzhi:xinzhi_grayscale:1`)后重建
- 业务表只增不删——验证脚本零破坏

### 3.3 共存零影响

- 40号 P7 实机回归(同一容器, 同轮执行): **33/33 全绿**
- 67号模块: git diff 零改动(68号只读引用 BANNED_KEYWORDS/
  carbon_profile/whitepaper)
- 46号 SCORER_REGISTRY: 新增 1 档案(xinzhi_merchant), 既有 40 档案零影响

---

## 四、运行时数据资产口径

### 4.1 七表用途与增长特性

| 表 | 写入时机 | 增长特性 |
|----|---------|---------|
| xinzhi_radar_snapshots | 每次雷达计算 | 每人可多快照(历史曲线 90 日) |
| xinzhi_product_scores | 每次评分批次 | 商品×会员组合幂等锚 |
| xinzhi_price_details | 每次价格拆解 | 留痕(杀熟审计源) |
| xinzhi_feedback | 每次反馈提交 | L1 自动闭环/L2/L3 工单 |
| xinzhi_groupbuys | 求购发布 | 三单上限控制 |
| xinzhi_merchants | 商家认证 | 一人一档幂等锚 |
| xinzhi_grayscale | 运行时态 | **单例(stateId=1)** |

### 4.2 序列化防护(P6g-4 实机教训)

- bool 显式还原(circuitBroken/closed/certified/paused 等 12 字段)
- 复杂字段(list/dict)注册序列化清单(responders/gradeHistory/
  trajectory/topRisks/roadmap/metrics/breachTrail 等 24 字段)
- `_list()` 通配排除 `:seq` 序列键(36号 attract 教训)

---

## 五、宪法域红线运行时验证

| 红线 | 运行时行为 | 实机证据 |
|------|-----------|---------|
| 杀熟零容忍 | 价差>20% → auditFlag 留痕(处置经 46号, 永不自动) | trusted vs restricted 实测 |
| α 只紧不松 | S.15/A.12/B.08 硬编码(XINZHI_ALPHA 常量) | 实机 α=0.15 |
| 地板不击穿 | α 抵扣后 max(final, 268×0.7) | promo 0.5 实测 |
| 降级永不自动 | 降档仅 46号 pending 建议书, 等级恒定 | 专项 changeId 留痕 |
| 零个体数据 | 邻里频道响应无任何身份字段 | 实机 flat 扫描 |
| 观测面永不关停 | off 态雷达/白皮书 200 | 实机 |
| 碳不可交易 | methodology 公示"只读观测不可交易" | 实机 |
| LLM 禁数字 | 导购话术数字与查询层一致 | 实机(¥/等级直出) |

---

## 六、运维操作速查

| 场景 | 操作 |
|------|------|
| 灰度放量 | `POST /api/xinzhi/mode/override?mode=shadow` → 观察 → `assist` |
| 紧急回退 | `POST /api/xinzhi/mode/override?mode=off`(或清除回落 env) |
| 护栏触发后 | 确认指标回落 → `POST /api/xinzhi/mode/resume`(留痕) |
| 状态巡检 | `GET /api/xinzhi/mode`(态+护栏+检查/违规计数) |
| 年度报告 | `GET /api/xinzhi/whitepaper`(四章节, 零个体数据) |
| 降档裁决 | 46号审批总线(`xinzhi_merchant` 档案 pending 建议书) |
| 实机验收 | `python backend/verify_xinzhi_p5_live.py`(56 断言) |

---

## 七、遗留与后续

| 项 | 状态 | 说明 |
|----|------|------|
| 生产部署(zxjiu.com) | ✅ 已完成(2026-09-11) | 13 文件 scp 同步+compose 注入 XINZHI_MODE+容器重建; 容器内 56/56 断言全绿; 公网 mode/白皮书/人格卡/邻里/求购大厅 200、guide 决策面 409(off 默认); Redis 业务数据零影响(857 键保留); 验证测试键已清理(22 xinzhi 键+测试会员) |
| 前端「臻选」频道 | 待实施 | 规划含 taro-app 一级频道(首页入口) |
| HTML/PDF 文档版 | ✅ 已完成 | 三册 HTML+PDF(提交 9c99a4d) |
| XINZHI_MODE 生产值 | 当前 off | 建议放量: `shadow`(观察期)→ 指标达标后 `assist`(override 切换免重建) |

---

## 八、结论

68号五期在 Docker Redis 容器实机环境完成 **56 断言×3 轮幂等全绿**:
五维雷达→臻选货架→透明定价(杀熟审计实测)→导购 SOP→邻里生态→
商家体系(46号总线)→灰度护栏→白皮书全链贯通; 与 40号(33/33)、
67号(零改动)共存零影响; 零影响铁律(off 默认)与观测面永不关停
(宪法口径)在实机环境逐项验证通过。**具备生产部署条件。**
