...
> 报告日期: 2026-09-16(终版刷新) | 范围: 前端消费面审计 → 发布工作台实施 → 生产实机验证 → 存量测试漂移修复
> 环境: 生产 47.236.61.117(zxjiu.com, NEXUSFLOW74_MODE=full) + 本地开发
> 结论: 74号从"纯后端发布引擎"完成运营面全触达; 存量测试漂移排查清零, 全站测试无已知失败项
...
| 项 | 结果 |
|---|---|
| test-nexus74.js(API 层) | 6/6(sources URL+admin 头/请求体映射/adapt/publish/receipt/平台字典) |
| test-nexus74.js(页面层) | 5/5(四页签/合规+警示注入/B 档回执/409 友好降级/LLM 禁入文案) |
| **前端合计** | **11/11** |
| 后端 test_nexus74_p1.py | 50/50(规则中枢/合规引擎) |
| 后端 test_nexus74_p2.py | 39/39(素材源/平台适配) |
| 后端 test_nexus74_p3.py | 48/48(发布调度/A-B 档) |
| 后端 test_nexus74_p4.py | 35/35(指标回流/学习) |
| 后端 test_nexus74_p5.py | 39/39(漂移监控/模型状态) |
| 后端 test_nexus74_p6.py | 39/39(复盘进化) |
| **后端合计** | **250/250** |
| H5 构建 | webpack 成功(app.420621bd.js, 含 nexus74 页面 chunk) |
| 生产部署 | dist 上传 /var/www/zxjiu/dist |
| 生产实机 | 四页签实时数据 + 入口全渲染(本文 §4.3) |

### 5.2 存量测试漂移专项排查与修复实录(提交 1b7c22f)

报告初版后执行"修复遗留项中的时间漂移测试"(同 73号 范式),
全站专项排查结果:
...
| 模块 | 结果 |
|---|---|
| 73号 P1/P2(上次修复对象) | 54+31 全绿, `_rel()` 相对日期修复无回归 |
| attract72 P3 | 50 全绿(ANCHOR/NOW 锚定模式, 设计正确不漂移) |
| 68号 xinzhi P0-P5 | 146 全绿 |
| 71号 pay71 P0-P8 | 483 全绿 |
| 74号 nexus74 P1-P6 | 250 全绿 |
| blogger/alliance/dm61/governance/decision/chat_transfer | 全绿 |

**计数漂移类——发现并修复 10 处**: batch3-7 五文件中
`len(SCORER_REGISTRY) == 56` 精确计数断言腐化(实际 64)。
根因: 62号升级注册 8 个新评分器后注册表 56→64; 62号当时仅修复
本模块(av62)可见失败, batch 存量未扫。

修复范式(62号 教训):

- 普通计数 → 下限断言 `>= 56`(batch3/4/5/6 + batch7 联动)
- 分布类 → **注册表守恒断言** `== len(SCORER_REGISTRY)`
  (batch7 全景总数/模式分布/默认 observe/恢复 observe——
  永随注册表自适应)

修复后 batch 家族 252 断言全绿(45+41+39+79+48), 改动行
ruff 干净。

**教训(与 73号 时间漂移教训同源)**: 模块升级改动共享注册表后,
应全站扫描同注册表的精确计数断言——精确计数会随后续模块
注册而腐化, 下限/守恒断言才具备时间不变性。

## 六、遗留项与建议(终态)

1. ~~存量测试漂移~~ **已清零**(本文 §5.2, 提交 1b7c22f)
2. **发布物管理深化**(可选): 发布记录当前为列表+回执, 可扩展
   详情/重试(自愈)操作面(retry API 已备)
3. **观测面/元认知运营面**(可选): immunity/redteam/evolution/
   drift 等元认知端点暂未建消费页, 建议后续按需扩展
4. **规则库管理页**(可选): rules 录入/字典为 admin 面, 当前经
   API 可用, 页面化优先级低
...
本轮完整提交链: **c52e8da**(工作台 7 文件 +1512 行) →
**47954e2**(报告初版) → **1b7c22f**(存量计数漂移修复 5 文件
10 处) → 本终版刷新。
