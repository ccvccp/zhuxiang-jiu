...
- ~~百度地图 AK 未配置: 前端走列表降级(诚实不伪造)~~ → **已于 2026-09-12 配置激活**(浏览器端 AK, Referer 白名单 zxjiu.com/*): 真实瓦片渲染 + 11 POI 标注实测通过; 过程修复两处注入缺陷(① zt.ts 防御式 `typeof process` 守卫破坏 DefinePlugin 整表达式替换→改为静态引用; ② 百度 v3.0 loader 动态插入时 document.write 失效→改 JSONP callback 加载)
