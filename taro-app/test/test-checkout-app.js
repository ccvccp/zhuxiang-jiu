/**
 * test-checkout-app.js · Taro 版 checkout-service APP 环境测试
 * ============================================================
 * 用 Node.js 模拟 APP 环境(React Native + Taro API),测试 9 阶段事务
 * ============================================================
 */

// ========== 1. 模拟 APP 环境 Taro API ==========
const asyncStorage = {};       // APP 端用 AsyncStorage（模拟）
const appRequestLog = [];      // APP 端网络请求日志

// 模拟 Taro API(APP 端 = React Native + AsyncStorage)
const mockTaro = {
  // 存储API(APP 端: AsyncStorage 同步接口)
  getStorageSync(key) {
    return asyncStorage[key] !== undefined ? asyncStorage[key] : '';
  },
  setStorageSync(key, data) {
    asyncStorage[key] = data;
  },
  removeStorageSync(key) {
    delete asyncStorage[key];
  },
  // 网络请求(APP 端: Taro.request → fetch/XMLHttpRequest)
  request(opts) {
    appRequestLog.push({ url: opts.url, method: opts.method, data: opts.data });
    // 模拟成功响应
    setTimeout(() => {
      opts.success({ statusCode: 200, data: { ok: true, message: 'Taro.request(APP) 模拟成功' } });
    }, 10);
  },
  // 环境标识
  getEnv() {
...
async function runTest() {
  ...
  console.log('Taro 版 checkout-service APP 环境测试');
  console.log('环境: ' + envAdapter.getEnv() + ' (APP / React Native)');
  console.log('='.repeat(60));
  ...
  checks.push({ 验证: '环境检测', env, 结果: env === 'rn' ? '✓ PASS' : '✗ FAIL env=' + env });
  ...
  // 存储路由(通过 Taro API = asyncStorage)
  const hasInAsyncStorage = asyncStorage[STORAGE_KEY] !== undefined && asyncStorage[SHIPPING_KEY] !== undefined;
  checks.push({
    验证: '存储路由(Taro API→AsyncStorage)',
    checkout_in_async: asyncStorage[STORAGE_KEY] !== undefined,
    shipping_in_async: asyncStorage[SHIPPING_KEY] !== undefined,
    结果: hasInAsyncStorage ? '✓ PASS' : '✗ FAIL'
  });

  // 事务轨迹完整(BEGIN→COMMIT)
  const txTrace = dbAfter.tx_log.map(t => t.type).join('→');
  checks.push({
    验证: '事务轨迹完整(BEGIN→COMMIT)',
    trace: txTrace,
    结果: txTrace.includes('BEGIN') && txTrace.includes('COMMIT') ? '✓ PASS' : '✗ FAIL'
  });
  ...
  console.log('测试结果汇总');
  console.log('='.repeat(60));
  console.log('环境: ' + env + ' (APP / React Native)');
  ...
  console.log('结论: ' + (passCount === checks.length ? '✓ APP 端 9 阶段事务全部通过(EnvAdapter 路由 Taro API 正常)' : '✗ 存在失败'));
  console.log('');
  ...
  // 存储路由验证
  console.log('存储路由验证(Taro API → AsyncStorage):');
  console.log('  AsyncStorage 中的 key:');
  Object.keys(asyncStorage).forEach(k => {
    console.log('    ' + k + ' (存在)');
  });
  console.log('');

  // 网络请求验证
  console.log('网络请求日志(APP 端 Taro.request):');
  console.log('  请求数: ' + appRequestLog.length);
  appRequestLog.forEach((r, i) => {
  ...
  if (passCount === checks.length) {
    console.log('✅ 全部 ' + checks.length + '/' + checks.length + ' PASS — Taro 版 checkout-service APP 环境测试通过');
  } else {
