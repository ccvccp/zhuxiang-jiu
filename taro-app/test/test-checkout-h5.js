/**
 * test-checkout-h5.js · Taro 版 checkout-service H5 环境测试
 * ============================================================
 * 用 Node.js 模拟 H5 环境(Taro API),测试 9 阶段事务
 * ============================================================
 */

// ========== 1. 模拟 H5 环境 Taro API ==========
const h5Storage = {};          // H5 端用 localStorage（模拟）
const h5RequestLog = [];       // H5 端网络请求日志

// 模拟 Taro API(H5 端 = localStorage + fetch)
const mockTaro = {
  // 存储API(H5 端: localStorage 同步接口)
  getStorageSync(key) {
    return h5Storage[key] !== undefined ? h5Storage[key] : '';
  },
  setStorageSync(key, data) {
    h5Storage[key] = data;
  },
  removeStorageSync(key) {
    delete h5Storage[key];
  },
  // 网络请求(H5 端: Taro.request → fetch/XMLHttpRequest)
  request(opts) {
    h5RequestLog.push({ url: opts.url, method: opts.method, data: opts.data });
    setTimeout(() => {
      opts.success({ statusCode: 200, data: { ok: true, message: 'Taro.request(H5) 模拟成功' } });
    }, 10);
  },
  getEnv() {
    return 'h5';
  ...
  async request(opts) {
    return new Promise((resolve, reject) => {
      mockTaro.request({
        ...
        success: (res) => {
          resolve({ ok: res.statusCode >= 200 && res.statusCode < 300, status: res.statusCode, json: () => Promise.resolve(res.data), data: res.data });
        },
...
const CheckoutService = {
  ...
  async submit(params) {
    ...
    try {
      ...
      const ctx = {
        orderNo: 'ZX' + Date.now(), items: params.items, memberId: params.memberId,
        memberLevel: params.memberLevel, points: params.points || 0,
        couponCode: params.couponCode, paymentMethod: params.paymentMethod || 'wechat', region: params.region,
      };
      ...
      ctx.priceResult = { originalTotal, memberDiscount: memberDiscountAmount, couponDiscount, pointsDeduct, finalAmount: finalAmount + shipping, shipping };
...
async function runTest() {
  ...
  console.log('Taro 版 checkout-service H5 环境测试');
  console.log('环境: ' + envAdapter.getEnv() + ' (H5 / 浏览器)');
  console.log('='.repeat(60));
  ...
  checks.push({ 验证: '环境检测', env, 结果: env === 'h5' ? '✓ PASS' : '✗ FAIL env=' + env });
  ...
  checks.push({ 验证: '阶段3-订单创建+发货方路由', shipper_type: order ? order.shipper_type : 'null', shipper_agent: order ? order.shipper_agent_name : 'null', ship_region: order ? order.ship_region : 'null', 结果: order && order.shipper_type === 'agent' ? '✓ PASS' : '✗ FAIL' });
  ...
  const hasInH5Storage = h5Storage[STORAGE_KEY] !== undefined && h5Storage[SHIPPING_KEY] !== undefined;
  checks.push({ 验证: '存储路由(Taro API→localStorage)', checkout_in_h5: h5Storage[STORAGE_KEY] !== undefined, shipping_in_h5: h5Storage[SHIPPING_KEY] !== undefined, 结果: hasInH5Storage ? '✓ PASS' : '✗ FAIL' });
  ...
  console.log('结论: ' + (passCount === checks.length ? '✓ H5 端 9 阶段事务全部通过(EnvAdapter 路由 Taro API 正常)' : '✗ 存在失败'));
  console.log('');
  console.log('逐项验证:');
  ...
  if (passCount === checks.length) {
    console.log('✅ 全部 ' + checks.length + '/' + checks.length + ' PASS — Taro 版 checkout-service H5 环境测试通过');
  } else {
