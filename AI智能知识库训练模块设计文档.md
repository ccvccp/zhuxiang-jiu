# AI智能知识库训练模块设计文档 v2.3

> **版本**: v2.3（P0~P5.3 全部已实现：RAG/Embedding/多模态视觉/抓取清洗/视频抽帧+ASR/Rerank 重排 + 生产化部署/监控缓存/前端接线/供应链事务 + 前端 live 补全/统一鉴权登录/Redis 生产底座）
> **状态**: 全阶段开发完成（P0~P2.5 + P3.x 检索链全系列 + P4 生产化四阶段 P4.1~P4.4 + P5 收尾闭环三阶段 P5.1~P5.3，最终推送 CI 两工作流全绿）
...
## 7.5 P4 规划（生产化阶段，2026-08-30 与用户对齐）

> **背景**：知识库后端能力（P0~P3.7）已完备，但盘点发现四个生产化断点——本阶段从"能力建设"转向"生产落地"。四项全选，按依赖顺序实施：P4.1 部署加固 → P4.2 监控缓存 → P4.3 前端接线 → P4.4 供应链。
...
| **P4.2 监控与缓存** | **已实施**：① 应用级监控 `core/metrics.py`（纯标准库线程安全 Counter/Histogram，不引入 prometheus_client）——HTTP 中间件埋点（QPS/状态码/延迟直方图，/metrics 自身不计）+ LLM 埋点（`llm_timer` 上下文：chat/vision/embed/transcribe/rerank 五方法调用计数 ok/error + 延迟直方图，埋点在 llm_client 内统一接入）+ RAG 缓存命中计数（layer=embedding/result × hit=yes/no）；`GET /metrics` 端点输出 Prometheus 文本格式（HELP/TYPE/标签/桶 le 累计/count/sum），可被既有 Prometheus 栈直接抓取；直方图仅计入最紧桶、渲染时累计（标准桶语义，修复了双重累计虚高）。② RAG 检索缓存（模块级 `_CACHES`，`rag_answer` 重构为缓存包装 + `_rag_answer_uncached`）——查询向量缓存（5 分钟 TTL，容量 1000，超限丢最旧一半；向量仅依赖 query 文本故条目变更不失效）+ 结果缓存（60 秒 TTL，key=问题+provider，容量 500；`_save_entry` 全量失效保知识新鲜度；缓存命中计数联动只发生一次视为首次结果延伸）；`_reset_knowledge_caches()` 供测试隔离。验证：test_app_metrics.py 8 项（Counter/Histogram/文本格式/llm_timer 双路径//metrics 端点/中间件埋点/自身豁免）+ test_knowledge_routes.py 新增 4 项缓存行为用例（结果命中只计一次 hit/条目变更失效/向量复用 embed 只调一次/命中打点），全量 133 项 + pytest 1154 项通过 | 高 | **已实施** |
| **P4.3 前端接线** | **已实施**：① 知识库管理后台两页（仿 ai-learning 单文件模式，原生 JS 零依赖，`X-Role: admin` 兼容头直连）——`knowledge-dashboard.html` 运营台（总览统计卡+质量调度器状态条+迁移/种子/重建索引一键运维；条目治理 Tab：状态 chip 筛选+创建候选+审核通过拒绝+发布+退役+pending/rejected 编辑重提+版本历史弹窗；缺口队列 Tab：三态筛选+补知识关联 entryId+忽略+高频缺口摘要）+ `knowledge-sources.html` 接入台（教学接入 Tab：创建会话+列表+会话内提问/教学提交闭环；文档·媒体·抓取 Tab：文档分块入库+图片 rule/llm 双轨+视频时间轴 JSON 分段+种子源白名单+内容接入/抓取运行；质量进化 Tab：淘汰扫描+质量报表+渐进信任过审+分发建议；RAG 检索测试 Tab：ask 置信分级路由透出 mode 徽章/置信度/引用列表+search 不计命中检索）。② 客服聊天窗 `js/chat-widget.js`（零依赖浮动组件：💬 FAB+面板，挂载 `ChatWidget.mount({apiBase})` 或 data-api-base 属性；会话创建/历史加载/发消息；**透出 ragMode 徽章（精确命中/知识融合/FAQ）+aiConfidence 置信度+citations 引用溯源列表（P3.2 成果用户可见）**；转人工（关键词自动+按钮）+满意度 1-5 星评价+新会话；X-Member-Id 随机会员身份 localStorage 持久化），挂载于 service.html。③ CORS 开发默认白名单补 `null` origin（file:// 直开）+ ALLOW_HEADERS 补 `X-Admin-Id`（knowledge 管理端点预检放行），生产经 CORS_ORIGINS 环境变量收紧。浏览器端到端验证（opaque origin 等价 file://）：三页面数据加载/RAG direct 命中 0.5103+引用/聊天窗精确命中徽章+置信度+引用全通过，pytest 1154 项全过 | 高 | **已实施** |
| **P4.4 供应链补齐** | **已实施**：四件套后端从"演示级简化实现"（单品、无事务编排）全面对齐前端已验证事务能力（契约复刻 js 四服务 mock 返回形状）。① 基础设施：`services/tx_utils.py`（TxLog 阶段日志 + acquire_locks 多锁升序获取防死锁 + 事务响应包装）+ `repositories/supply_chain_repository.py`（双模式 List/Hash/全量域仓库）+ store.py 扩展 18 个供应链数据域（注意 `checkout_points` 不能叫 `points_accounts`——与积分模块惰性初始化探针键冲突）。② inventory：多行 deduct/restock（先校验后执行+攒批落库+防御性补偿回滚；inventory_logs 流水+低库存预警 0<stock≤10；单行附旧契约兼容字段 stockAfter/txId；单品请求兼容）。③ checkout：9 阶段事务（订单创建+发货方路由→库存扣减→优惠券核销→积分扣减→积分入账（等级加成）→分润（平台80/酒店20）+厂家→代理商5%同品服务费→支付确认→攒批落库；锁 order/coupon/points 升序；失败逆序补偿全量回滚；P3 价格计算复刻 order-pricing.js：会员折扣→满减→券→积分抵扣≤30%→运费两瓶免）。④ warehouse 10 端点全量（inbound 入库上架库位分配/outbound 出库核销/stocktake 实盘覆盖盈亏汇总/slot-optimize ABC 重排/forecast LSTM 预测+补货建议/multi-transfer 多仓调拨双向流水/loss 损耗登记/cross-dock 越库不动库存/safety-stock 动态安全库存+再订货点/env-monitor 温湿度监控酒龄管理；锁 stock:{wid}:{pid}+wh:{wid}；AI 状态机 sufficient/normal/low/critical）。⑤ shipping：claim 富契约（claimId/details/asyncOps+兼容字段）+新增 release 端点（认领状态机 已认领→已退出→可再认领）+resolve_shipper 发货方解析+accrue_service_fee 服务费计提（随 checkout 事务原子提交）。验证：test_supply_chain.py 新增并发不变量测试（不超卖/库存守恒/事务零污染/一区一代理）+ 旧契约测试全量更新，pytest 1158 全过 + ruff 全过 | 中 | **已实施** |
...
| 方向 | 内容 | 优先级 | 状态 |
|---|---|---|---|
| **P5.1 前端 live 补全** | **已实施**：① env-adapter.js H5 fetch GET 分支丢参修复——data 序列化为 query string 拼接 url（对齐小程序分支），warehouse 三个 GET 端点（forecast/safety-stock/env-monitor）live 模式恢复传参能力（验证：传自定义 productId/warehouseId 返回对应非默认数据）。② 后端新增两查询端点——`GET /api/inventory/stock?productId=`（库存查询，404 产品不存在）+ `GET /api/agent-shipping/settlement?agentId=`（服务费结算统计：聚合 service_fees 的 totalCount/pendingCount/pendingAmount/settledAmount/settledAs 同品，兼容顶层平铺）+ claims 端点加 `?detail=true` 参数（返回富记录数组含已退出认领，默认保持旧 {region: agentId} 映射向后兼容）；supply_chain_repository 补 hgetall 原语。③ agent-shipping 前端补全 live 分支——通用 liveRequest helper（非 2xx 解析后端错误体 {success:false, error} 保持与 mock 失败形状一致）+ claim/release 事务入口 live 分支（响应形状与后端 P4.4 富契约一致）+ listClaims/getServiceFeeSettlement 双模式（mock 同步返回/live 返回 Promise）；清理 liveClaim 死代码。④ inventory getStock 双模式（mock 同步返回 number|null/live 返回 Promise，liveGetStock 查询后端 stock 端点）。验证：test_supply_chain.py 新增 10 项端点测试（stock 查询/404/422/扣减反映/settlement 空态聚合隔离/claims detail 富记录+默认兼容）全过；浏览器端到端（源码注入真实执行三 JS 模块 + live 模式实测）：EnvAdapter GET query 序列化修复验证（自定义参数返回非默认数据）+ InventoryService live getStock 与 checkout 扣减闭环（500→498）+ AgentShippingService live claim/release/listClaims/settlement 全链路（服务费 55.9=1118×5% 精确匹配）；pytest 1168 全过 + ruff 全过 | 高 | **已实施** |
| **P5.2 统一鉴权登录** | **已实施**：补上鉴权最后一块拼图，`AUTH_MODE=strict` 下全站可用。① `js/auth.js` 统一 Token 管理工具——会员 JWT 为统一凭证（login 换 accessToken+refreshToken 双令牌存 localStorage 'zhuxiang.auth'，本地过期检查提前 60s）；管理员登录两段式（先 POST /api/admin/login 校验后台凭证含 2FA 检查，再用绑定会员手机号换统一 JWT——因 admin 会话 token 与 JWT 中间件并行体系不兼容，且会员表中已有 role=admin 账号）；apiHeaders() 统一注入 Authorization Bearer 头 / requireLogin() 未登录跳登录页（redirect 参数回跳）/ mountBadge() 登录态 UI（昵称+管理员标识+退出按钮）/ refresh() 刷新令牌 / logout() 本地清除+后端黑名单 best-effort。② login.html 登录页——双 Tab（会员/管理员，管理员含 totpCode+绑定手机号字段）；防开放跳转（redirect 参数仅允许站内相对文件名，拒绝 //host、协议前缀、javascript: 伪协议——indexOf(':')<0 校验）。③ 五个管理页接入——knowledge-dashboard/knowledge-sources/ai-learning-dashboard/ai-learning-detail/ai-learning-report 各页 apiHeaders() 合并 Auth.apiHeaders()（登录后 Bearer 头覆盖 compat 头，未登录保留 X-Role 向后兼容）+ fetch 封装 401 分支（会话失效跳登录页）+ init 入口拦截（未登录 requireLogin）+ 页头 authBadge 登录态 UI 挂载。验证：compat 模式 10 项（登录/role=admin JWT 访问管理端点/错密码 409/兼容头仍可用/auth.js 模块生命周期/五页接入+login.html 防跳转）+ **strict 模式 7 项**（无头 401/仅 X-Role 401 兼容头失效/登录白名单可用/Bearer 全链路 200/伪造 token 401/auth.js 衔接/防跳转含伪协议）——三角闭环：未登录拦截+旧通道封死+JWT 唯一合法通道 | 高 | **已实施** |
| **P5.3 Redis 生产补齐** | **已实施**：生产化数据底座闭环。① seed_redis.py 补齐供应链四件套 19 个数据域灌注——`seed_supply_chain()` 复用 store.py `_build_initial_supply_chain()` 同一数据源（Hash 域：checkout_coupons 2 券 + checkout_points 5 等级积分，值 JSON 序列化；全量域：supply_warehouses 4 仓 + warehouse_locations 180 库位 + warehouse_stock 14 条仓储库存，逐条 JSON RPUSH 且幂等先清后写；shipping_claim_details 与 13 个 List 域初始为空懒创建）+ `verify_supply_chain_seed()` 独立验证函数接入主流程与 verify 输出。② test_redis_integration.py 新增两组测试类（共 16 项）：`TestSupplyChainRepositoryRedis`（List/Hash/全量域三组原语的 Redis 读写往返——append→list_all JSON 透明往返/remove_last 弹出与空列表/hget hset hdel hgetall 往返/hget_int 默认值/load→改→save 整体回写不损其余记录/非法域名 ValueError）+ `TestSupplyChainServiceRedis`（四件套服务层 Redis 关键路径——inventory deduct_lines 库存+流水落库/shipping claim→release 状态机/warehouse outbound 全量域读改写出库单落库/checkout 9 阶段事务库存扣减+订单分润落库），配套 sc_seeded_redis fixture。③ 本地验证：环境无 Redis/Docker/WSL，采用 fakeredis 2.37 注入双客户端单例（repositories.backend + core.locks）跑通 11 项全链路（seed 198 条域数据+三组原语往返+四件套服务层关键路径）；真实 Redis 集成由 CI redis-integration-tests job 兜底、Docker 容器联调由 CI docker job 兜底（本地 Docker Desktop 未运行，与 P4.1 时一致的环境限制）。全量回归 pytest 1168 + ruff 全过。**CI 闭环**：首推 fa76817 后 CI redis-integration job 抓出 sc_seeded_redis fixture 漏灌普通数据域（clear 清空后仅灌 `zhuxiang:sc:*`，inventory 扣减/shipping claim/checkout 3 用例失败——教训首版只记在更新日志未落到 fixture 代码），修复为 fixture 内补灌 seed_agents+seed_inventory（c7bbc15），fakeredis 注入双单例精确复现失败场景验证修复后重推，CI 与 AI Scoring & Flash Sale 两工作流全绿（真实 Redis 集成测试复核通过） | 中 | **已实施** |
...
| P5.1 前端 live 补全 | 2026-08-30 | env-adapter H5 GET query 参数序列化修复（warehouse 三 GET 端点恢复传参）+ 后端 inventory/stock 与 agent-shipping/settlement 查询端点 + claims detail 富记录；agent-shipping 前端 live 分支补全（liveRequest helper + claim/release/listClaims/settlement，清理死代码）+ inventory getStock 双模式；浏览器端到端源码注入实测三 JS 模块 live 全链路 |
| P5.2 统一鉴权登录 | 2026-08-30 | js/auth.js 统一 Token 管理（会员 JWT 双令牌本地过期检查 + 管理员两段式后台凭证→绑定手机号换 JWT；apiHeaders/requireLogin/mountBadge/refresh/logout）+ login.html 双 Tab 登录页（防开放跳转拒绝协议前缀与伪协议）+ 五管理页接入（Bearer 头覆盖 compat/401 跳登录/init 拦截/登录态 UI）；compat 10 项 + strict 7 项三角闭环验证（未登录拦截+旧通道封死+JWT 唯一合法通道） |
| P5.3 Redis 生产补齐 | 2026-08-30 | seed_supply_chain 19 域灌注（复用 _build_initial_supply_chain 同一数据源，Hash/全量域 JSON 序列化幂等先清后写）+ verify_supply_chain_seed 独立验证 + Redis 集成测试 16 项（三组原语读写往返 + 四件套服务层关键路径）+ fakeredis 双单例注入本地全链路验证；CI 真实 Redis 兜底抓出 fixture 漏灌普通域并修复（补灌 seed_agents+seed_inventory），重推 CI 与 AI Scoring 两工作流全绿 |
