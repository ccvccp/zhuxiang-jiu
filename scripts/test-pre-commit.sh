...
TL_HOOK="$REPO_ROOT/.git/hooks/pre-commit"
...
# TC1: 输入 Y → 通过
tl_run_hook_case "TC1: auto-input Y -> PASS" "Y" 0 \
    "git add '$TEST_FILE' >/dev/null 2>&1"

# TC2: 输入 n → 阻止
tl_run_hook_case "TC2: auto-input n -> BLOCK" "n" 1 \
    "git add '$TEST_FILE' >/dev/null 2>&1"

# TC3: 输入 N → 阻止
tl_run_hook_case "TC3: auto-input N -> BLOCK" "N" 1 \
    "git add '$TEST_FILE' >/dev/null 2>&1"

# TC4: 空输入/EOF → 通过 (response="" 非 n/N)
tl_run_hook_case "TC4: empty input/EOF -> PASS" "" 0 \
    "git add '$TEST_FILE' >/dev/null 2>&1"

# TC5: 无 JS 暂存 → 跳过
tl_run_hook_case "TC5: no staged JS -> SKIP" "" 0 \
...
# ---------- 汇总 ----------
tl_print_summary "pre-commit CI 确认流程单元测试 (5 cases)"
exit $?
